...
mod tests {
    ...
    async fn parse_text_delta() {
        let sse = "data: {\"id\":\"chatcmpl-test\",\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"finish_reason\":null,\"index\":0}],\"model\":\"gpt-4o\"}\n\n";
        let response = mock_sse_response(sse).await;
        let mut stream = parse_openai_sse_stream(response);
    ...
    async fn parse_done_terminates_stream() {
        let sse = "data: {\"id\":\"chatcmpl-test\",\"choices\":[{\"delta\":{\"content\":\"Hi\"},\"finish_reason\":null,\"index\":0}],\"model\":\"gpt-4o\"}\n\ndata: [DONE]\n\n";
        let response = mock_sse_response(sse).await;
        let mut stream = parse_openai_sse_stream(response);
    ...
    async fn parse_tool_call_delta() {
        let sse = "data: {\"id\":\"chatcmpl-test\",\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"call_abc\",\"type\":\"function\",\"function\":{\"name\":\"bash\",\"arguments\":\"{\\\"cmd\\\":\"}}]},\"finish_reason\":null,\"index\":0}],\"model\":\"gpt-4o\"}\n\n";
        let response = mock_sse_response(sse).await;
        let mut stream = parse_openai_sse_stream(response);
    ...
    async fn parse_finish_reason_with_usage() {
        let sse = "data: {\"id\":\"chatcmpl-test\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\",\"index\":0}],\"model\":\"gpt-4o\",\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":20,\"total_tokens\":30}}\n\n";
        let response = mock_sse_response(sse).await;
        let mut stream = parse_openai_sse_stream(response);
