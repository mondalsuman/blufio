...
pub async fn get_entity_classification(
    ...
    let table = table_for_entity(entity_type).ok_or_else(|| {
        BlufioError::Internal(format!("unknown entity type: {entity_type}"))
    })?;
...
pub async fn bulk_update_classification(
    ...
    db.connection()
        .call(move |conn| {
            ...
            let build_conditions = |start_idx: usize| -> (Vec<String>, Vec<Box<dyn rusqlite::types::ToSql>>) {
            ...
            if dry_run {
                ...
                let count: usize = conn.query_row(&sql, params_slice.as_slice(), |row| row.get(0))?;
...
mod tests {
    ...
    async fn get_classification_returns_correct_level_for_message() {
        ...
        let result = get_entity_classification(&db, "message", "msg-1").await.unwrap();
    ...
    async fn get_classification_returns_correct_level_for_session() {
        ...
        let result = get_entity_classification(&db, "session", "sess-cls").await.unwrap();
    ...
    async fn get_classification_returns_none_for_nonexistent() {
        let (db, _dir) = setup_db().await;
        let result = get_entity_classification(&db, "message", "no-such-msg").await.unwrap();
    ...
    async fn set_classification_updates_and_returns_true() {
        ...
        let updated = set_entity_classification(&db, "message", "msg-1", "restricted").await.unwrap();
        assert!(updated);
    ...
    async fn set_classification_returns_false_for_nonexistent() {
        let (db, _dir) = setup_db().await;
        let updated = set_entity_classification(&db, "message", "no-such", "public").await.unwrap();
        assert!(!updated);
    }
    ...
    async fn list_by_classification_with_filter() {
        ...
        let internal = list_entities_by_classification(&db, "message", Some("internal")).await.unwrap();
        assert_eq!(internal.len(), 2);
        assert!(internal.iter().all(|(_, c)| c == "internal"));

        let confidential = list_entities_by_classification(&db, "message", Some("confidential")).await.unwrap();
    ...
    async fn list_by_classification_without_filter() {
        ...
        let all = list_entities_by_classification(&db, "message", None).await.unwrap();
        assert_eq!(all.len(), 2);
    ...
    async fn bulk_update_dry_run_counts_without_modifying() {
        ...
        let result = bulk_update_classification(
            &db, "message", "confidential",
            Some("internal"), None, None, None, None, true,
        ).await.unwrap();
        assert_eq!(result.total, 2);
        assert_eq!(result.succeeded, 0);

        // Verify nothing was modified.
        let check = get_entity_classification(&db, "message", "msg-1").await.unwrap();
    ...
    async fn bulk_update_applies_update_and_returns_count() {
        ...
        let all = list_entities_by_classification(&db, "message", Some("restricted")).await.unwrap();
        assert_eq!(all.len(), 3);
    }
    ...
    async fn bulk_update_with_current_level_filter() {
        ...
        let result = bulk_update_classification(
            &db, "message", "restricted",
            Some("internal"), None, None, None, None, false,
        ).await.unwrap();
        assert_eq!(result.succeeded, 1);

        // Only msg-1 should be restricted now.
        let check1 = get_entity_classification(&db, "message", "msg-1").await.unwrap();
        assert_eq!(check1, Some("restricted".to_string()));

        let check2 = get_entity_classification(&db, "message", "msg-2").await.unwrap();
