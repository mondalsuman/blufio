...
use crate::types::{
    ChatContent, ChatMessage, ContentPart, FunctionCall, FunctionDef, ImageUrlData, OpenAITool,
    ProviderPreferences, RouterRequest, StreamOptions, ToolCall,
};

/// OpenRouter provider implementing [`ProviderAdapter`].
///
/// Supports both synchronous completion and streaming responses via SSE.
/// Routes requests through OpenRouter's unified API, with configurable
/// provider preferences (order, fallbacks) and analytics headers.
///
...
impl OpenRouterProvider {
    ...
    ///
    /// # API Key Resolution
    /// 1. `config.providers.openrouter.api_key` if set
    ...
    pub async fn new(config: &BlufioConfig) -> Result<Self, BlufioError> {
        let api_key = resolve_api_key(&config.providers.openrouter.api_key)?;
        ...
        let client = OpenRouterClient::new(
            ...
            config.providers.openrouter.x_title.clone(),
            config.providers.openrouter.http_referer.clone(),
            Some(&config.security),
        )?;

        info!(
            model = config.providers.openrouter.default_model,
            x_title = config.providers.openrouter.x_title,
            provider_order = ?config.providers.openrouter.provider_order,
        ...
        Ok(Self {
            client,
            system_prompt,
            provider_order: config.providers.openrouter.provider_order.clone(),
        })
    }
    ...
    /// Converts a [`ProviderRequest`] to an OpenRouter [`RouterRequest`].
    ///
    /// Key mappings:
    ...
    /// - `provider_order` -> `provider.order` field (OpenRouter-specific)
    fn to_router_request(&self, request: &ProviderRequest) -> RouterRequest {
        ...
        // Build provider preferences only if provider_order is non-empty.
...
impl PluginAdapter for OpenRouterProvider {
    ...
    async fn health_check(&self) -> Result<HealthStatus, BlufioError> {
        // Defer validation to first actual request -- avoid consuming tokens
        // on health checks. OpenRouter does not have a zero-cost auth endpoint.
...
impl ProviderAdapter for OpenRouterProvider {
    async fn complete(&self, request: ProviderRequest) -> Result<ProviderResponse, BlufioError> {
        let api_request = self.to_router_request(&request);
        ...
        // Map finish_reason to provider stop_reason.
...
/// Maps an OpenRouter SSE chunk to zero or more `ProviderStreamChunk`s.
///
/// Handles:
...
/// Maps OpenAI/OpenRouter `finish_reason` to provider-agnostic `stop_reason`.
...
/// Converts a [`ProviderMessage`] into one or more OpenRouter [`ChatMessage`]s.
///
...
fn resolve_api_key(config_key: &Option<String>) -> Result<String, BlufioError> {
    ...
    std::env::var("OPENROUTER_API_KEY").map_err(|_| {
        BlufioError::Config(
            "OpenRouter API key not found. Set providers.openrouter.api_key in config or OPENROUTER_API_KEY environment variable.".into(),
        )
    })
}
...
mod tests {
    ...
    fn test_provider() -> OpenRouterProvider {
        ...
        .unwrap();
        OpenRouterProvider::with_client(client, "Test system prompt.".into(), vec![])
    }

    fn test_provider_with_order() -> OpenRouterProvider {
        let client = OpenRouterClient::new(
            "test-key".into(),
            "anthropic/claude-sonnet-4".into(),
            "Blufio".into(),
        ...
        .unwrap();
        OpenRouterProvider::with_client(
            client,
            "Test prompt.".into(),
    ...
    fn to_router_request_basic() {
        ...
        assert_eq!(router_req.model, "anthropic/claude-sonnet-4");
        assert_eq!(router_req.max_completion_tokens, Some(2048));
        assert!(router_req.stream);
        // First message is system, second is user.
        assert_eq!(router_req.messages.len(), 2);
        assert_eq!(router_req.messages[0].role, "system");
        match &router_req.messages[0].content {
            Some(ChatContent::Text(t)) => assert_eq!(t, "Test system prompt."),
            other => panic!("expected Text, got {other:?}"),
        }
        assert_eq!(router_req.messages[1].role, "user");
        // No provider preferences when order is empty.
        assert!(router_req.provider.is_none());
    }

    #[test]
    fn to_router_request_with_provider_order() {
        let provider = test_provider_with_order();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
            system_blocks: None,
        ...
        let router_req = provider.to_router_request(&request);
        let prefs = router_req.provider.as_ref().unwrap();
        assert_eq!(prefs.order, vec!["Anthropic", "Google"]);
        assert!(prefs.allow_fallbacks);
    }

    #[test]
    fn empty_provider_order_omits_provider_field() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        // Also verify serialization omits it.
        let json = serde_json::to_value(&router_req).unwrap();
        assert!(json.get("provider").is_none());
    }

    #[test]
    fn to_router_request_uses_explicit_system_prompt() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
    ...
    #[test]
    fn to_router_request_maps_tool_definitions() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        let router_req = provider.to_router_request(&request);
        let tools = router_req.tools.as_ref().unwrap();
    ...
    #[test]
    fn to_router_request_maps_image_to_image_url() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        assert_eq!(router_req.messages.len(), 2);
        match &router_req.messages[1].content {
    ...
    #[test]
    fn to_router_request_maps_tool_use_to_assistant_tool_calls() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        assert_eq!(router_req.messages.len(), 2);
        assert_eq!(router_req.messages[1].role, "assistant");
        let tool_calls = router_req.messages[1].tool_calls.as_ref().unwrap();
    ...
    #[test]
    fn to_router_request_maps_tool_result_to_tool_message() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        assert_eq!(router_req.messages.len(), 2);
        assert_eq!(router_req.messages[1].role, "tool");
        assert_eq!(
            router_req.messages[1].tool_call_id.as_deref(),
            Some("call_abc")
        );
        match &router_req.messages[1].content {
    ...
    #[test]
    fn full_model_ids_passed_through() {
        let provider = test_provider();
        let request = ProviderRequest {
        ...
        let router_req = provider.to_router_request(&request);
        assert_eq!(router_req.model, "google/gemini-2.0-flash");
    }
    ...
    #[test]
    fn stream_options_set_when_streaming() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        let router_req = provider.to_router_request(&request);
        let opts = router_req.stream_options.as_ref().unwrap();
        assert!(opts.include_usage);
    }

    #[test]
    fn stream_options_absent_when_not_streaming() {
        let provider = test_provider();
        let request = ProviderRequest {
            model: "anthropic/claude-sonnet-4".into(),
            system_prompt: None,
        ...
        let router_req = provider.to_router_request(&request);
        assert!(router_req.stream_options.is_none());
    }
}
