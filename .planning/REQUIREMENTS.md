...
- [ ] **FOUND-01**: MCP config structs added to blufio-config with TOML parsing and deny_unknown_fields
- [ ] **FOUND-02**: Workspace crates blufio-mcp-server and blufio-mcp-client scaffolded with feature flags
- [ ] **FOUND-03**: rmcp 0.17.0 and schemars 1.0 added to workspace dependencies (verify reqwest version unification)
- [ ] **FOUND-04**: Tool namespace convention enforced in ToolRegistry with collision detection and built-in priority
- [ ] **FOUND-05**: MCP session ID type distinct from Blufio conversation session ID
- [ ] **FOUND-06**: rmcp abstraction boundary established (Blufio-owned types, no public rmcp re-exports)

### MCP Server

- [ ] **SRVR-01**: User can connect Claude Desktop to Blufio via stdio and list available tools
- [ ] **SRVR-02**: User can invoke Blufio skills from Claude Desktop via MCP tools/call
- [ ] **SRVR-03**: `blufio mcp-server` CLI subcommand for stdio-only mode (no agent loop)
- [ ] **SRVR-04**: Capability negotiation (initialize/initialized handshake) with MCP spec 2025-11-25
- [ ] **SRVR-05**: Tool input validation against inputSchema with JSON-RPC -32602 errors
- [ ] **SRVR-06**: Streamable HTTP transport mounted on existing gateway at /mcp
- [ ] **SRVR-07**: MCP-specific auth middleware for HTTP transport (bearer token)
...
- [ ] **SRVR-16**: CORS restricted to configured origins on MCP HTTP endpoints
